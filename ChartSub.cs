...
//		double interval = 0.1;
		...
		//	double	ep=0.1;
			...
			pDC.FillRectangle(/*brsBody*/Brushes.LightGreen,0, 0, pdef.rt_cl.right/*-2*/, pdef.rt_cl.bottom/*-2*/);
		#else
			pDC.FillSolidRect(&pdef.rt_cl, RGB(224,224,224));
		#endif
			pDC.FillRectangle(Brushes.White, pdef.rt_gr);
			Draw4Edge(pDC, pdef.rt_gr, Pens.Black);
			//--------------------------------
			rt = pdef.rt_gr;
		...
		//		pDC.SelectObject(&rpen);//太
				pDC.DrawLine(penBlak, pdef.rt_gr.left, tmp, pdef.rt_gr.left-SLEN, tmp);
				//pDC.MoveTo(pdef.rt_gr.left     , tmp);
				//pDC.LineTo(pdef.rt_gr.left-SLEN, tmp);
				if ((pdef.bitLBL & 2)!=0) {
					//pDC.DrawText(buf, -1, &rt, DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					DrawText(pDC, Brushes.Black, buf, rt, StringAlignment.Center, StringAlignment.Far);
				}
				if (bMAXMIN) {
					continue;
				}
		...
		//			pDC.SelectObject(&rpen);//太
					...
					pDC.DrawLine(penBlak, tmp, pdef.rt_gr.bottom, tmp, pdef.rt_gr.bottom+SLEN);
					if ((pdef.bitLBL & 1)!=0) {
						//pDC.DrawText(buf, -1, &rt, DT_SINGLELINE|DT_CENTER|DT_TOP);
						DrawText(pDC, Brushes.Black, buf, rt, StringAlignment.Near, StringAlignment.Center);
					}
					if (bMAXMIN) {
						continue;
					}
			...
			if (true) {
				...
				DrawText(pDC, Brushes.Black, buf, rt, StringAlignment.Center, StringAlignment.Near);
			}
