...
//								w, fWav[i0], fWav[i1], v));
		...
		static public String GRID2CSV(DataGridView dg)
		{
			return(GRID2CSV(dg, -1, -1));
		}
		/************************************************************/
		static public String GRID2CSV(DataGridView dg, int rows, int cols)
		{
...
//			object		obj;
			...
			/**/
			for (c = 0; c < cols; c++) {
				if (dg.ColumnHeadersVisible == false) {
					continue;
				}
				if (c > 0) {
					str.Append(",");
				}
				str.Append(dg.Columns[c].HeaderText.Replace("\r", ""));
			}
			str.Append("\r\n");
			/**/
			for (r = 0; r < rows; r++) {
...
#if true
					if (dg[c,r].Value == null) {
						tmp = "";
					}
					else {
						tmp = dg[c,r].EditedFormattedValue.ToString();
					}
